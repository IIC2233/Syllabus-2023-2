...
class Icono(QObject):
    ...
    def __init__(
        ...
        self.timer.setInterval(p.TIEMPO_MOVIMIENTO)
...
class ControladorLogico(QObject):
    ...
    def __init__(self, tamaño_simulacion: list, tamaño_icono: int):
        ...
        self.timer = QTimer(self)
        self.timer.setInterval(1000)  # 1000 ms == 1 segundo
    ...
    def empezar(self):
        ...
        # con su movimiento
        self.timer.start()
        iconos = list(self.iconos.values())
        for icono in iconos:
            icono.timer.start()
            icono.otros_enemigos = iconos
    ...
    def click_pantalla(self, x: int, y: int, tipo: str):
        ...
        # No olvidar conectar señales internas!
        icono.senal_choque.connect(self.choque)

        # Señal para crear un icono, manda id, x, y, tipo
        self.senal_aparecer_icono.emit(icono.id, icono.x, icono.y, icono.tipo)
        self.iconos[icono.id] = icono
        if self.empece:
            # Agregar icono a mitad de la simulacion
            for icono in self.iconos.values():
                icono.otros_enemigos.append(icono)
            icono.otros_enemigos = list(self.iconos.values())
            icono.timer.start()

    def choque(self, enemigo_id: int, tipo: str):
        # backend actualiza el tipo del enemigo
        self.iconos[enemigo_id].tipo = tipo
        # le avisamos al frontend que actualice la imagen del icono
        self.senal_actualizar_icono.emit(enemigo_id, tipo)

        # Extra, verificar si ya terminó la simulación
        tipos = set()
        for icono in self.iconos.values():
            tipos.add(icono.tipo)
    ...
    def poblar_random(self):
        for _ in range(p.CANTIDAD_RANDOM):
            ...
            tipo = random.choice(["anticucho", "empanada", "choripan", "comida4"])
